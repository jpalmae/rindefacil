...
DEFAULT_CLOUD_PROMPT = """Analiza la imagen del recibo y devuelve EXACTAMENTE un JSON, sin texto adicional, sin explicaciones y sin markdown.

Formato requerido:
{
  "amount": <número>,
  "currency": <"CLP", "USD" o "PEN", o null si no es clara>,
...
Reglas estrictas:
- "amount" es el total pagado en números, sin símbolos ni separadores de miles. Si el monto está en pesos chilenos, es un número entero. Ejemplos: 1990, 50. Si está en soles peruanos (S/) o dólares (USD), respeta los decimales con punto. Ejemplo: "S/ 1,250.50" → 1250.50. Ejemplo: "USD 299.00" → 299.
- "currency" identifica la moneda del documento: "CLP" para pesos chilenos ($ o CLP), "USD" para dólares (US$, USD, $ en facturas internacionales), "PEN" para soles peruanos (S/). Si no puedes determinarla con certeza, devuelve null.
...
DEFAULT_LOCAL_PROMPT = DEFAULT_CLOUD_PROMPT
